...
#define JABLOTRON_PWM_SIZE (JABLOTRON_RAW_SIZE * 2)  // half-bit entries for diphase
 ...
 *
 * Each bit produces two half-bit PWM entries (counter_top = 32 each,
 * totalling 64 carrier cycles = one RF/64 bit period).
 *
...
static const nrf_pwm_sequence_t *jablotron_modulator(jablotron_codec *d, uint8_t *buf) {
    ...
    bool level = false;  // starting signal level
    ...
    for (int i = 0; i < JABLOTRON_RAW_SIZE; i++) {
        ...
        // First half-bit
        m_jablotron_pwm_seq_vals[2 * i].channel_0 = (level ? (1 << 15) : 0) | 32;
        m_jablotron_pwm_seq_vals[2 * i].counter_top = 32;
        ...
        // Second half-bit
        m_jablotron_pwm_seq_vals[2 * i + 1].channel_0 = (level ? (1 << 15) : 0) | 32;
        m_jablotron_pwm_seq_vals[2 * i + 1].counter_top = 32;
    }
